class Spinach::Features::TestPostEndpoint < Spinach::FeatureSteps
  step "I create an Item" do
    HTTParty.post("http://localhost:3000/api/v1/items", {body: {name: "Foo", quality: 4, sellIn: 4, description: "Bar"}})
  end

  step "I create another Item" do
    HTTParty.post("http://localhost:3000/api/v1/items", {body: {name: "Bar", quality: 5, sellIn: 5, description: "Foo"}})
  end

  step "I create a third Item" do
    HTTParty.post("http://localhost:3000/api/v1/items", {body: {name: "FooBar", quality: 6, sellIn: 6, description: "BarFoo"}})
  end
